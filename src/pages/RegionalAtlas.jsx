...
const territoryWord = n => n % 100 >= 11 && n % 100 <= 14 ? "территорий" : n % 10 === 1 ? "территория" : n % 10 >= 2 && n % 10 <= 4 ? "территории" : "территорий";
...
export default function RegionalAtlas({ region, focusCity }) {
  ...
  const [category, setCategory] = useState(() => categories.find(c => meta.categories[c.id])?.id || ""), [query, setQuery] = useState(""), [stageId, setStageId] = useState("");
  ...
  useEffect(() => {
    ...
    setError(false);
    fetch(new URL(`content/atlas/${region.id}.json`, siteRoot), { signal: controller.signal }).then(r => { if (!r.ok) throw new Error("Atlas unavailable"); return r.json(); }).then(result => { setData(result); const object = result.objects.find(o => o.id === projectSearch().get("object")); if (object) { setPlanId(object.planId); setCategory(object.category); setSelected(object.id); } }).catch(e => { if (e.name !== "AbortError") setError(true); });
  ...
  const activePlan = meta.plans.find(p => p.id === planId);
  const categoryCount = categories.filter(c => meta.categories[c.id]).length;
  ...
  return <section id="projects" className="projects-section regional-atlas-section" aria-labelledby="projects-title" ref={section}>
    ...
    <div className={`regional-atlas-stage ${expanded ? "is-expanded" : ""} ${object ? "has-selection" : ""}`} ref={stage} role={expanded ? "dialog" : "region"} aria-modal={expanded ? true : undefined} aria-label={`Атлас проектов — ${region.name}`} style={{ "--atlas-accent": activeCategory?.color || "#79dffa", "--accent": activeCategory?.color || "#79dffa" }}>
      ...
      <aside className="regional-atlas-sidebar">
        <div className="regional-sidebar-head"><div className="regional-sidebar-eyebrow"><span className="section-kicker">Атлас развития</span><button aria-label="Поиск и фильтры проектов" aria-expanded={filtersOpen} aria-controls={`atlas-filters-${region.id}`} onClick={() => setFiltersOpen(!filtersOpen)}><Search size={19} /></button></div><h3>{region.name}</h3><span>{meta.count} {objectWord(meta.count)} / {categoryCount} направлений</span></div>
        <div className="regional-atlas-filters" id={`atlas-filters-${region.id}`} hidden={!filtersOpen}><label className="regional-atlas-search"><Search size={19} /><input aria-label="Найти объект в регионе" placeholder="Найти объект" value={query} onChange={e => changeFilter(() => setQuery(e.target.value))} /><button aria-label="Очистить поиск" hidden={!query} onClick={() => changeFilter(() => setQuery(""))}><Icon name="close" size={18} /></button></label>
        ...
        <div className="regional-atlas-categories" aria-label="Направления развития региона">{categories.filter(c => meta.categories[c.id]).map(c => { const count = data ? data.objects.filter(o => o.category === c.id && (!planId || o.planId === planId)).length : meta.categories[c.id]; return <button key={c.id} className={category === c.id ? "is-active" : ""} aria-pressed={category === c.id} disabled={!count} style={{ "--category-color": c.color }} onClick={() => changeFilter(() => setCategory(c.id))}><Icon name={c.icon} size={20} /><span>{c.title}</span><small>{count}</small></button>; })}</div>
        <div className="regional-list-heading"><span aria-live="polite">{activeCategory?.title || "Все проекты"} <b>{data ? shown.length : meta.categories[category] || meta.count}</b></span>{(query || planId || stageId) && <button onClick={reset}>Сбросить</button>}</div>
      ...
      <div className="regional-map-toolbar"><span className="regional-map-count">{meta.count} {objectWord(meta.count)} · {meta.plans.length} {territoryWord(meta.plans.length)}</span><button aria-label="Приблизить карту" disabled={!mapReady} onClick={() => api.current?.zoom(1)}><Icon name="plus" size={22} /></button><button aria-label="Отдалить карту" disabled={!mapReady} onClick={() => api.current?.zoom(-1)}><Icon name="minus" size={22} /></button><button aria-label="Общий вид региона" disabled={!mapReady} onClick={reset}><Icon name="reset" size={21} /></button><button ref={expandButton} aria-label={expanded ? "Закрыть полный экран" : "Открыть карту на весь экран"} aria-expanded={expanded} onClick={() => setExpanded(!expanded)}><Icon name={expanded ? "close" : "expand"} size={21} /></button></div>
      {!object && <div className="atlas-invitation regional-atlas-invitation"><span className="atlas-invitation-count">{String(data ? shown.length : meta.categories[category] || meta.count).padStart(2,"0")}</span><div><span>{activePlan?.name || "проектов направления"}</span><h3>{activeCategory?.title || "Развитие региона"}</h3><button disabled={!shown.length} onClick={() => choose(shown[0].id)}>Исследовать<Icon name="arrow" hoverName="right" size={22} /></button></div></div>}
