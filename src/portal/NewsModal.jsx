...
export function NewsLink({ post, children, onOpen, ...props }) {
...
export function NewsProvider({ children }) {
  ...
  useEffect(() => {
    const sync = () => setId(selectedNews());
    window.addEventListener("popstate", sync);
    return () => window.removeEventListener("popstate", sync);
  }, []);
  const open = (next, element, replace = false) => {
    if (element) trigger.current = element;
    const url = new URL(location.href);
    url.searchParams.set("news", next);
    if (replace) history.replaceState(history.state, "", url);
    else history.pushState({ ...history.state, newsModal: true }, "", url);
    setId(next);
  };
  const close = () => {
  ...
  return <NewsContext.Provider value={open}>
    {children}
    {post && <NewsDialog post={post} onClose={close} onChange={next => open(next, null, true)} trigger={trigger} />}
...
function NewsDialog({ post, onClose, onChange, trigger }) {
    ...
    <div className="p-news-dialog-frame">
      ...
      <div className="p-news-dialog-scroll" ref={scroller} onScroll={updateProgress}>
        ...
        <footer className="p-news-dialog-next">
          <button disabled={index === 0} onClick={() => onChange(news[index - 1].id)}><ArrowLeft size={19} />Предыдущая новость</button>
          <button disabled={index === news.length - 1} onClick={() => onChange(news[index + 1].id)}>Следующая новость<ArrowRight size={19} /></button>
        </footer>
      </div>
    </div>
  </dialog>;
